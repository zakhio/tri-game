...
const initialState: GameState = {
    token: localStorage.getItem("token") || uuid(),
...
export const joinAsync = (token: string, sessionId: string, navigate?: NavigateFunction): AppThunk => (dispatch, getState) => {
...
// will call the thunk with the `dispatch` function as the first argument. Async
// code can then be executed and other actions can be dispatched
export const createSession = (token: string, navigate: NavigateFunction, language?: string): AppThunk => dispatch => {
